...
#[tokio::test]
async fn test_pay_sui_success_one_input_coin2() -> anyhow::Result<()> {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let object_id = ObjectID::random();
    let coin_amount = 50000000;
    let coin_obj = Object::with_id_owner_gas_for_testing(object_id, sender, 50000000);
    ...
    // make sure the first object still belongs to the sender,
    // the value is equal to all residual values after amounts transferred.
...
async fn execute_pay_sui_direct(
    input_coin_objects: Vec<Object>,
    recipients: Vec<SuiAddress>,
    amounts: Vec<u64>,
    sender: SuiAddress,
    ...
    // Use PaySui directly instead of ProgrammableTransaction
