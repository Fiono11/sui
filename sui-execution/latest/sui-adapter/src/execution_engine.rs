...
mod checked {
    ...
    pub fn transfer_coin(
        ...
        // Extract the inner type from Coin<T> to pass to new_coin, which expects just T
        // (it will wrap it in Coin<...> itself). This ensures we create Coin<SUI>
        // instead of Coin<Coin<SUI>>.
        let inner_type = coin_type
            .type_params
            .first()
            .expect("Coin type must have one type parameter")
            .clone();
        let new_coin = Object::new_move(
            MoveObject::new_coin(inner_type, SequenceNumber::new(), *coin.id(), coin.value()),
    ...
    pub fn update_input_coins(
        ...
        temporary_store.mutate_input_object(gas_coin_obj);
                    ...
                    if let Some(typ) = &coin_type {
                        if typ != &struct_tag {
                            return Err(ExecutionError::new_with_source(
                                ExecutionErrorKind::InvariantViolation,
                                format!(
                                    "Coin type check failed in pay/pay_sui/pay_all_sui transaction, expected: {:?}, found: {:?}",
                                    typ, struct_tag
                                ),
                            ));
                        }
    ...
    fn check_total_coins(coins: &[Coin], amounts: &[u64]) -> Result<(u64, u64), ExecutionError> {
        ...
        if total_amount > total_coins {
            return Err(ExecutionError::new_with_source(
                ...
                format!(
                    "Attempting to pay a total amount {:?} that is greater than the sum of input coin values {:?}",
